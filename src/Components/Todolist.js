...
function Todolist(props) {
    const todolist = props.todolist.map((task,index) => {
        const taskComplete = task => {
            axios.put(`https://cap-todo-list-app.herokuapp.com/api/tasks/${task._id}` , {
                _id : task._id,
                todo: task.todo,
        ...
        const removeTask = id => {
            axios.delete(`https://cap-todo-list-app.herokuapp.com/api/tasks/${id}`).then(res => props.removeTask(res.data)).catch(err => console.log(err))
        } 
        return <li key = {index}>
